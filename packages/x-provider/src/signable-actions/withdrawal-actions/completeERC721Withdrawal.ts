...
async function completeERC721RegisterAndWithdrawal(
  ...
  return mintsApi
    ...
    .then((mintableToken) =>
      executeMintableERC721RegisterAndWithdraw(
        ethSigner,
        starkSigner,
        {
    ...
    .catch((error) => {
      if (error.response?.status === 404) {
        ...
        return executeERC721RegisterAndWithdraw(
          ethSigner,
          starkSigner,
          token,
          config
...
async function completeERC721Withdrawal(
  ...
  return mintsApi
    ...
    .then((mintableToken) =>
      executeMintableERC721Withdrawal(
        ethSigner,
        ownerKey,
        {
          type: ERC721TokenType,
    ...
    .catch((error) => {
      if (error.response?.status === 404) {
        // token is already minted on L1
        return executeERC721Withdrawal(ethSigner, ownerKey, token, config);
      }
      throw error; // unable to recover from any other kind of error
    });
}
...
export async function completeERC721WithdrawalAction(
  ...
  const { v3Balance, v4Balance } = await getWithdrawalBalancesERC721(
    ethSigner,
    starkPublicKey,
    ethAddress,
    {
