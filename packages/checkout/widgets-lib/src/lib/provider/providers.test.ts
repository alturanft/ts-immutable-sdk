import { ExternalProvider, BrowserProvider } from "ethers";
  ...
  describe("createAndConnectToProvider", () => {
    it("should return provider", () => {
      const mockCheckout = {
        createProvider: jest
          .fn()
          .mockResolvedValue({ provider: mockPassportWeb3Provider }),
        checkIsWalletConnected: jest
          .fn()
      ...
      const result = createAndConnectToProvider(
        mockCheckout,
    ...
    it("should throw error when createProvider fails", () => {
      const mockCheckout = {
        createProvider: jest
          .fn()
          .mockRejectedValue(new Error("Failed to create provider")),
      } as unknown as Checkout;
      const result = createAndConnectToProvider(
        mockCheckout,
        WalletProviderName.PASSPORT
    ...
    it("should throw error when checkIsWalletConnected fails", () => {
      const mockCheckout = {
        ...
        checkIsWalletConnected: jest
          .fn()
          .mockRejectedValue(new Error("Failed to checkIsWalletConnected")),
      } as unknown as Checkout;
      const result = createAndConnectToProvider(
        mockCheckout,
        WalletProviderName.PASSPORT
