...
describe("ConnectWidget tests", () => {
  ...
  beforeEach(() => {
    ...
    providerRemoveListenerStub = cy
      .stub()
      .as("providerRemoveListenerStub")
      .returns({});
  });
    ...
    mount(
      <ViewContextTestComponent>
        <ConnectWidget
          ...
          checkout={checkout}
          {...props}
          web3Provider={mockBrowserProvider}
        />
      </ViewContextTestComponent>
    );
  };
  ...
  const mountConnectWidgetWithPassport = (
    passportProviderRequest: "reject" | "resolve"
  ) => {
    ...
    mount(
      <ViewContextTestComponent>
        <ConnectWidget config={config} checkout={checkout} />
      </ViewContextTestComponent>
    );
  };

  const mountConnectWidgetWithPassportAndGoToReadyToConnect = (
    passportProviderRequest: "reject" | "resolve"
  ) => {
          ...
          .resolves({
            provider: {
              provider: {
                ...
                removeListener: providerRemoveListenerStub,
                request: async () =>
                  Promise.resolve(ChainId.IMTBL_ZKEVM_TESTNET), // used for checking chainId
              },
              getSigner: () => ({
                getAddress: () => Promise.resolve(""),
                getChainId: async () =>
                  Promise.resolve(ChainId.IMTBL_ZKEVM_TESTNET),
              }),
            } as unknown as BrowserProvider,
          });
        cy.stub(Checkout.prototype, "createProvider")
    ...
    describe("Switch", () => {
      ...
      it("should show success when ready to connect pressed and network switched", () => {
        ...
        cy.stub(Checkout.prototype, "switchNetwork")
          ...
          .resolves({
            provider: mockBrowserProvider as BrowserProvider,
            network: {
  ...
  describe("Error Connecting", () => {
    it("should show error view if unable to create provider", () => {
      ...
      mount(
        <ViewContextTestComponent>
          <ConnectWidget {...props} checkout={checkout} config={config} />
        </ViewContextTestComponent>
      );
