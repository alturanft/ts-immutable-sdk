        ...
        <CryptoFiatProvider environment={environment}>
          ...
          {viewState.view.type === BridgeWidgetViews.BRIDGE_FAILURE && (
            <StatusView
              testId="bridge-fail"
              statusText={t(
                "views.BRIDGE_FAILURE.bridgeFailureText.statusText"
              )}
              actionText={t(
                "views.BRIDGE_FAILURE.bridgeFailureText.actionText"
              )}
          ...
          {viewState.view.type === BridgeWidgetViews.APPROVE_TRANSACTION && (
            <ApproveTransaction
              bridgeTransaction={
                viewState.view.approveTransaction && viewState.view.transaction
                  ? {
          ...
          {viewState.view.type ===
            BridgeWidgetViews.CLAIM_WITHDRAWAL_IN_PROGRESS && (
            <ClaimWithdrawalInProgress
              transactionResponse={viewState.view.transactionResponse}
            />
          )}
          {viewState.view.type ===
            BridgeWidgetViews.CLAIM_WITHDRAWAL_SUCCESS && (
            <StatusView
              statusText={t("views.CLAIM_WITHDRAWAL.IN_PROGRESS.success.text")}
              actionText={t(
                "views.CLAIM_WITHDRAWAL.IN_PROGRESS.success.actionText"
              )}
              onRenderEvent={() => {
                page({
                  userJourney: UserJourney.BRIDGE,
                ...
                sendBridgeClaimWithdrawalSuccessEvent(
                  eventTarget,
                  (viewState.view as BridgeClaimWithdrawalSuccess)
                    .transactionHash
                );
              }}
          ...
          {viewState.view.type ===
            BridgeWidgetViews.CLAIM_WITHDRAWAL_FAILURE && (
            <StatusView
              statusText={t("views.CLAIM_WITHDRAWAL.IN_PROGRESS.failure.text")}
              actionText={t(
                "views.CLAIM_WITHDRAWAL.IN_PROGRESS.failure.actionText"
              )}
              onRenderEvent={() => {
                let reason = "";
                if (
                ...
                sendBridgeClaimWithdrawalFailedEvent(
                  eventTarget,
                  (viewState.view as BridgeClaimWithdrawalFailure)
                    .transactionHash,
                  "Transaction failed"
                );
              }}
