...
import { Body, Box, Button, Heading, Icon, Logo, MenuItem } from "@biom3/react";
import {
...
export function BridgeReviewSummary() {
  ...
  const [estimates, setEstimates] = useState<
    GasEstimateBridgeToL2Result | undefined
  >(undefined);
  ...
  const [fromWalletIsWalletConnect, setFromWalletIsWalletConnect] =
    useState<boolean>(false);
  const [toWalletIsWalletConnect, setToWalletIsWalletConnect] =
    useState<boolean>(false);
  ...
  const insufficientFundsForGas = useMemo(() => {
    ...
    const nativeTokenBalance = tokenBalances.find((balance) =>
  ...
  const fetchTransferGasEstimate = useCallback(async () => {
    ...
    const tokenToTransfer =
      token?.address?.toLowerCase() ?? NATIVE.toUpperCase();
    ...
    if (tokenToTransfer === NATIVE.toLowerCase()) {
      estimatePromise = checkout.providerCall(
        from.web3Provider,
        async (provider) =>
      ...
      const erc20 = getErc20Contract(
        tokenToTransfer,
        from.web3Provider.getSigner()
      );
      estimatePromise = erc20.estimateGas.transfer(
        toAddress,
        utils.parseUnits(amount, token.decimals)
      );
    }
  ...
  useEffect(() => {
    if (isWalletConnectEnabled) {
      const isFromProviderWalletConnect = isWalletConnectProvider(
        from?.web3Provider
      );
      const isToProviderWalletConnect = isWalletConnectProvider(
        to?.web3Provider
      );
  ...
  const submitBridge = useCallback(async () => {
    ...
    try {
      const currentChainId = await (from?.web3Provider.provider as any).request(
    ...
    <Box testId={testId} sx={bridgeReviewWrapperStyles}>
      ...
      >
        ...
        <MenuItem.Label
          sx={{
            marginLeft:
              fromWalletIsWalletConnect && fromWalletLogoUrl ? "0px" : "45px",
          }}
        >
      ...
      >
        ...
        <MenuItem.Label
          sx={{
            marginLeft:
              toWalletIsWalletConnect && toWalletLogoUrl ? "0px" : "45px",
          }}
        >
