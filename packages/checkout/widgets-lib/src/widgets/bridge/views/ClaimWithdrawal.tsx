...
export function ClaimWithdrawal({ transaction }: ClaimWithdrawalProps) {
  ...
  const {
    bridgeState: { checkout, tokenBridge, from },
  ...
  const [withdrawalResponse, setWithdrawalResponse] =
    useState<FlowRateWithdrawResponse | null>();
  ...
  useEffect(() => {
    const getWithdrawalTxn = async () => {
      if (
        !tokenBridge ||
        !transaction ||
        transaction.details.current_status?.index === undefined
      )
        return;
      // get withdrawal transaction from the token bridge by receipient address and index
      setLoading(true);
      try {
        const flowRateWithdrawTxnResponse =
          await tokenBridge?.getFlowRateWithdrawTx({
  ...
  const handleWithdrawalClaimClick = useCallback(
    async ({ forceChangeAccount }: { forceChangeAccount: boolean }) => {
      if (
        !checkout ||
        !tokenBridge ||
        !from?.web3Provider ||
      ...
      if (
        !withdrawalResponse.pendingWithdrawal.canWithdraw ||
        !withdrawalResponse.unsignedTx
      ) {
        // eslint-disable-next-line max-len, no-console
        console.log(
          `Unable to process withdrawal transaction as it is not ready yet. Delay timeout at ${withdrawalResponse.pendingWithdrawal.timeoutEnd} `
        );
        return;
      }
      ...
      try {
        try {
          gasEstimate = await providerToUse.estimateGas(
            withdrawalResponse.unsignedTx
          );
        } catch (err) {
        ...
        if (feeData && feeData.maxPriorityFeePerGas) {
          gasPriceInWei = feeData.maxFeePerGas;
      ...
      try {
        ...
        const ethBalance = balancesResult.balances.find((balance) =>
  ...
  return (
          ...
          >
            ...
            >
              {loading || txProcessing ? (
                ...
                t(
                  `views.CLAIM_WITHDRAWAL.${
                    hasWithdrawError ? "footer.retryText" : "footer.buttonText"
                  }`
                )
              )}
            </Button>
    ...
    >
      <SimpleTextBody
        heading={`${t(
          "views.CLAIM_WITHDRAWAL.content.heading"
        )} ${getChainNameById(getL1ChainId(checkout.config))}`}
      >
        ...
        <Box>
          ...
          <EllipsizedText
            text={transaction.details.to_address.toLowerCase() ?? ""}
          />
        </Box>
      </SimpleTextBody>
