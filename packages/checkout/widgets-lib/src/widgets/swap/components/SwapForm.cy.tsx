...
describe("SwapForm", () => {
  ...
  beforeEach(() => {
    ...
    cryptoConversions = new Map<string, number>([
      ["eth", 1800],
      ["imx", 0.75],
    ]);
  ...
  describe("initial form state", () => {
    it("should show all swap inputs with initial state", () => {
      ...
      cySmartGet("fromTokenInputs-select-form-select__target").should(
        "be.visible"
      );
      cySmartGet("fromTokenInputs-select-form-select__target").should(
        "have.text",
      ...
      cySmartGet("fromTokenInputs-text-form-text__input").should(
        "have.attr",
        "placeholder",
        "0"
      );
      cySmartGet("toTokenInputs-select-form-select__target").should(
        "be.visible"
      );
      cySmartGet("toTokenInputs-select-form-select__target").should(
        "have.text",
  ...
  describe("swap form behaviour", () => {
    it("should validate from input when direction is from and Swap Button is clicked", () => {
      ...
      cySmartGet("toTokenInputs-text-form-text-control-error").should(
        "not.exist"
      );
    });
    ...
    it("should validate to input when direction is to and Swap Button is clicked", () => {
      ...
      cySmartGet("fromTokenInputs-text-form-text-control-error").should(
        "not.exist"
      );
    });
    ...
    it("should show insufficient balance error when swap from amount is larger than token balance", () => {
      ...
      cySmartGet(
        "fromTokenInputs-select-form-coin-selector__option-eth-0xf57e7e7c23978c3caec3c3548e3d615c346e79ff"
      ).click();
      cySmartGet("fromTokenInputs-text-form-text__input")
        .type("20")
        .trigger("change");
    ...
    it("should remove validation error when swap from token is chosen", () => {
      ...
      cySmartGet(
        "toTokenInputs-select-form-coin-selector__option-native"
      ).click();
      cySmartGet("fromTokenInputs-text-form-text__input").type("0.01").blur();
      cySmartGet("swap-button").click();
      cySmartGet("fromTokenInputs-select-form-select-control-error")
  ...
  describe("when to fetch a quote", () => {
    ...
    beforeEach(() => {
      fromAmountInStub = cy
        .stub(quotesProcessor, "fromAmountIn")
        .as("fromAmountInStub");
    ...
    it("should set to amount and fees after quote is fetched", () => {
      ...
      cySmartGet("@fromAmountInStub").should("have.been.calledWith", ...params);
      cySmartGet("fees-gas-fee__priceDisplay__price").should(
        "have.text",
        "≈ IMX 0.112300"
      );
      cySmartGet("fees-gas-fee__priceDisplay__fiatAmount").should(
        "have.text",
        "≈ USD $0.08"
      );
    });
  ...
  describe("submitting a swap", () => {
    describe("dex success", () => {
      ...
      it("should show adjust button if user does not have enough imx and imx is in from", () => {
        ...
        cySmartGet("toTokenInputs-select-form-select__target").click();
        cySmartGet(
          "toTokenInputs-select-form-coin-selector__option-eth-0xf57e7e7c23978c3caec3c3548e3d615c346e79ff"
        ).click();
        cySmartGet("fromTokenInputs-select-form-select__target").click();
        cySmartGet(
          "fromTokenInputs-select-form-coin-selector__option-native"
        ).click();

        cySmartGet("toTokenInputs-text-form-text__input")
          .type("0.00001")
          .trigger("change");
        cySmartGet("@fromAmountInStub").should("have.been.called");
        cySmartGet("swap-button").click();
      ...
      it("should show loading if user has enough imx and does not reject the transaction", () => {
        ...
        cySmartGet("swap-button__icon").should(
          "have.attr",
          "data-icon",
          "Loading"
        );
