...
export function sendWalletWidgetCloseEvent(eventTarget: Window | EventTarget) {
  const closeWidgetEvent = new CustomEvent<
    WidgetEvent<WidgetType.WALLET, WalletEventType.CLOSE_WIDGET>
...
export function sendNetworkSwitchEvent(
  ...
  console.log(
    ...
    eventTarget,
    walletWidgetSwitchNetworkEvent
  );
  if (eventTarget !== undefined)
    eventTarget.dispatchEvent(walletWidgetSwitchNetworkEvent);
}

export function sendDisconnectWalletEvent(eventTarget: Window | EventTarget) {
  const disconnectWalletEvent = new CustomEvent<
    WidgetEvent<WidgetType.WALLET, WalletEventType.DISCONNECT_WALLET>
  >(IMTBLWidgetEvents.IMTBL_WALLET_WIDGET_EVENT, {
    detail: {
