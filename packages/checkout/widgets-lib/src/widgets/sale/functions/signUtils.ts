...
export const filterAllowedTransactions = async (
  ...
  try {
    ...
    const approveTxn = transactions.find((txn) =>
      txn.methodCall.startsWith("approve")
    );
    ...
    const desiredAmount = approveTxn.params.amount
      ? BigNumber.from(approveTxn.params.amount)
      : BigNumber.from(0);

    const isAllowed =
      currentAmount.gte(BigNumber.from("0")) &&
      currentAmount.gte(desiredAmount);

    if (isAllowed) {
      return transactions.filter(
        (txn) => txn.methodCall !== approveTxn.methodCall
      );
    }
  } catch {
