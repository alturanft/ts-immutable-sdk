...
export const MainPage = () => {
  ...
  const [selectedTheme, setSelectedTheme] = useState<WidgetTheme>(
    WidgetTheme.DARK
  );
  ...
  const connectWidget = useMemo(
    () => widgetsFactory.create(WidgetType.CONNECT),
    [widgetsFactory]
  );
  const walletWidget = useMemo(
    () => widgetsFactory.create(WidgetType.WALLET),
    [widgetsFactory]
  );
  const bridgeWidget = useMemo(
    () => widgetsFactory.create(WidgetType.BRIDGE),
    [widgetsFactory]
  );
  const swapWidget = useMemo(
    () => widgetsFactory.create(WidgetType.SWAP),
    [widgetsFactory]
  );
  const onRampWidget = useMemo(
    () => widgetsFactory.create(WidgetType.ONRAMP),
    [widgetsFactory]
  );
  ...
  useEffect(() => {
    ...
    connectWidget.addListener(
      ...
      (eventData: ConnectionSuccess) => {
        setWeb3Provider(eventData.provider);
      }
    );
    walletWidget.addListener(
      WalletEventType.NETWORK_SWITCH,
      (eventData: WalletNetworkSwitch) => {
        setWeb3Provider(eventData.provider);
      }
    );
  ...
  const openConnectWidget = useCallback(
    (targetChainId?: ChainId, blockWallets: boolean = false) => {
      const connectParams: {
  ...
  const updateLanguage = useCallback(
    (language: string) => {
      ...
      const languageUpdate: WidgetProperties<any> = {
        config: { language },
      } as WidgetProperties<any>;
  ...
  const toggleTheme = useCallback(() => {
    ...
    const themeUpdate: WidgetProperties<any> = {
      config: { theme },
    } as WidgetProperties<any>;
  ...
  return (
    <Box
      sx={{
        ...
        width: "100%",
        height: "100%",
        backgroundColor: "base.color.brand.6",
      }}
    >
      <Box
        sx={{
      ...
      >
        ...
        >
          ...
          <Button onClick={() => openConnectWidget(undefined, true)}>
            Connect (Blocked)
          </Button>
          ...
          <LanguageSelector
            onLanguageChange={(language: string) => updateLanguage(language)}
        ...
        {passport &&
          ...
          (web3Provider.provider as any)?.isPassport && (
            <Button onClick={logout}>Passport Logout</Button>
          )}
      </Box>
      <Box
        sx={{
