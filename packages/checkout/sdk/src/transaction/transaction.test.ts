...
describe("transaction", () => {
  ...
  it("should return unpredictable gas limit request status if transaction errors with unpredictable gas limit", async () => {
    const mockProvider = {
      getNetwork: jest.fn().mockReturnValue({
        chainId: ChainId.ETHEREUM,
      } as NetworkInfo),
  ...
  it("should include txn gas limits for zkEVM chains if the gasPrice is not defined on the transaction", async () => {
    const transactionResponse = {
      hash: "123",
      from: "0x234",
      confirmations: 5,
