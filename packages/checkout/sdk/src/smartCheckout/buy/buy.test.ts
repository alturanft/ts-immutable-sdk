...
import { ActionType, TransactionPurpose, constants } from "@imtbl/orderbook";
import { BrowserProvider } from "ethers";
import { getItemRequirement, buy, getTransactionOrGas } from "./buy";
import {
  createBlockchainDataInstance,
  createOrderbookInstance,
  getTokenContract,
} from "../../instance";
...
describe("buy", () => {
  ...
  describe("buy", () => {
    ...
    it("should call smart checkout with item requirements and execute transactions - ERC721 order", async () => {
      ...
      const order: BuyOrder = {
        id: "1",
        takerFees: [
          { amount: { percentageDecimal: 0.025 }, recipient: "0xFEERECIPIENT" },
        ] as OrderFee[],
      };
      ...
      expect(signFulfillmentTransactions).toBeCalledWith(mockProvider, [
        { from: "0xTRANSACTION" },
      ]);
    ...
    it("should call smart checkout with item requirements and execute transactions for ERC20 fulfillment - ERC721 order", async () => {
      ...
      (getUnsignedFulfillmentTransactions as jest.Mock)
        .mockRejectedValueOnce(
          new Error("Cannot estimate gas - not enough ERC20 approval")
        )
    ...
    "should call smart checkout with item requirements and throw error if building fulfillment transaction fails after approving", async () => {
      const smartCheckoutResult = {
        sufficient: true,
        transactionRequirements: [
          {
      ...
      (getUnsignedFulfillmentTransactions as jest.Mock).mockRejectedValue(
        new Error("Cannot estimate gas - not enough ERC20 approval")
      );
      (signApprovalTransactions as jest.Mock).mockResolvedValue({
        type: SignTransactionStatusType.SUCCESS,
    ...
    it("should call smart checkout with item requirements and gas limit if fulfillOrder errors with balance error", async () => {
      (getUnsignedERC20ApprovalTransactions as jest.Mock).mockResolvedValue([
        { from: "0xAPPROVAL" },
      ]);
      (getUnsignedFulfillmentTransactions as jest.Mock).mockResolvedValue([]);
      ...
      const order: BuyOrder = {
        id: "1",
        takerFees: [
          { amount: { percentageDecimal: 0.01 }, recipient: "0xFEERECIPIENT" },
        ],
      };
      const itemRequirements = [
        {
  ...
  describe("getItemRequirement", () => {
    it("should return type of native and amount", () => {
      ...
      const tokenAddress = "";
      const result = getItemRequirement(
        type,
        tokenAddress,
        amount,
        seaportContractAddress
  ...
  describe("getTransactionOrGas", () => {
    ...
    it("should get gas amount if no fulfillment transaction", () => {
      expect(getTransactionOrGas(gasLimit, [])).toEqual({
        type: TransactionOrGasType.GAS,
        gasToken: {
          type: GasTokenType.NATIVE,
          limit: BigNumber.from(gasLimit),
