...
describe("erc1155", () => {
  ...
  describe("getSetERC1155ApprovalForAllTransaction", () => {
    it("should get the approval transaction from the contract with the from added", async () => {
      const setERC1155ApprovalForAllTransactionMock = jest
        .fn()
        .mockResolvedValue({ data: "0xDATA" });
      (Contract as unknown as jest.Mock).mockReturnValue({
        populateTransaction: {
      ...
      expect(setERC1155ApprovalForAllTransactionMock).toBeCalledWith(
        "0xSEAPORT",
        true
      );
    });
    ...
    it("should throw checkout error if call to approve fails", async () => {
      const setERC1155ApprovalForAllTransactionMock = jest
        .fn()
        .mockRejectedValue({ from: "0xADDRESS" });
      (Contract as unknown as jest.Mock).mockReturnValue({
        populateTransaction: {
          setApprovalForAll: setERC1155ApprovalForAllTransactionMock,
        },
  ...
  describe("hasERC1155Allowances", () => {
    it("should return allowances with sufficient false and approval transaction if allowance not sufficient", async () => {
      const isApprovedForAllMock = jest.fn().mockResolvedValue(false);
      const approveTxMock = jest
        .fn()
        .mockResolvedValue({ data: "0xDATA", to: "0x00000" });
      (Contract as unknown as jest.Mock).mockReturnValue({
        isApprovedForAll: isApprovedForAllMock,
        populateTransaction: {
      ...
      const allowances = await hasERC1155Allowances(
        mockProvider,
        "0xADDRESS",
        itemRequirements
      );
