...
export const getSetERC1155ApprovalForAllTransaction = async (
  ...
  try {
    ...
    const transaction = await contract.setApprovalForAll.populateTransaction(
      spenderAddress,
      true
    );
...
export const hasERC1155Allowances = async (
  ...
  const erc1155ItemRequirements: ERC1155ItemRequirement[] =
    itemRequirements.filter(
      (itemRequirement): itemRequirement is ERC1155ItemRequirement =>
        itemRequirement.type === ItemType.ERC1155
    );
