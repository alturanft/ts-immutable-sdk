...
describe("erc721", () => {
  ...
  describe("hasERC721Allowances", () => {
    it("should return allowances with sufficient false and approval transaction if allowance not sufficient", async () => {
      ...
      const allowances = await hasERC721Allowances(
        mockProvider,
        "0xADDRESS",
        itemRequirements
      );
    ...
    it("should return sufficient false for non-approved items and true for contract addresses that are approved for all", async () => {
      ...
      const approveMock = jest
        .fn()
        .mockResolvedValue({ data: "0xDATA", to: "0xOTHER" });
      (Contract as unknown as jest.Mock).mockReturnValue({
        getApproved: getApprovedMock,
        isApprovedForAll: isApprovedForAllMock,
        populateTransaction: {
