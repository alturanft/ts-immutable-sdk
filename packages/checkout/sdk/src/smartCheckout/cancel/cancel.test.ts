...
describe("cancel", () => {
  ...
  describe("on chain cancel", () => {
    ...
    it("should throw error when sign rejects", async () => {
      ...
      (signFulfillmentTransactions as jest.Mock).mockRejectedValue(
        new Error("ERROR")
      );

      await expect(cancel(config, mockProvider, [orderId])).rejects.toThrow(
        "ERROR"
      );
  ...
  describe("gasless cancel", () => {
    ...
    it("should handle errors from orderbook", async () => {
      ...
      (createOrderbookInstance as jest.Mock).mockReturnValue({
        prepareOrderCancellations: jest
          .fn()
          .mockRejectedValue(new Error("Error from orderbook")),
      });
