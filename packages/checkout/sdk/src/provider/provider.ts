...
export async function createProvider(
  ...
  switch (walletProviderName) {
    case WalletProviderName.PASSPORT: {
      providerDetail = InjectedProvidersManager.getInstance().findProvider({
        rdns: passportProviderInfo.rdns,
      });
      if (!providerDetail) {
        if (passport) {
          web3Provider = new BrowserProvider(
            passport.connectEvm({ announceProvider: false })
          );
        } else {
    ...
    case WalletProviderName.METAMASK: {
      providerDetail = InjectedProvidersManager.getInstance().findProvider({
        rdns: metaMaskProviderInfo.rdns,
      });
      if (!providerDetail) {
