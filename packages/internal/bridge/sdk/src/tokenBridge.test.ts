...
describe("Token Bridge", () => {
  ...
  describe("getUnsignedBridgeBundledTx", () => {
    ...
    beforeEach(() => {
      jest
        .spyOn(TokenBridge.prototype as any, "getDynamicDepositGas")
        .mockImplementation(async () => ({
          approvalGas: 50000,
          sourceChainGas: 120000,
        }));
      jest
        .spyOn(mockRootProvider, "getFeeData")
        .mockImplementation(async () => ({
    ...
    it("returns the both approval tx and bridge tx when allowance is insufficient for ERC20", async () => {
      ...
      jest
        .spyOn(TokenBridge.prototype as any, "getAllowance")
      ...
      const expectedData = erc20Contract.interface.encodeFunctionData(
        "approve",
        [bridgeConfig.bridgeContracts.rootERC20BridgeFlowRate, amount]
      );

      expect(result.contractToApprove).toBe(
        "0x2f14582947E292a2eCd20C430B46f2d27CFE213c"
      );
      ...
      expect(result.unsignedBridgeTx.value).toBe(
        ethers.utils.parseUnits("0.0000000001", 18).toString()
      );
    ...
    it("returns the only bridge tx for NATIVE", async () => {
      ...
      expect(result.unsignedBridgeTx.value).toBe(
        ethers.utils.parseUnits("0.0000000001", 18).add(amount).toString()
      );
      expect(result.unsignedBridgeTx.data).not.toBeNull();
    });
  });
  ...
  describe("getUnsignedBridgeTx", () => {
    ...
    it("ERC20 token with valid arguments is successful", async () => {
      ...
      const response: BridgeTxResponse = await tokenBridge.getUnsignedBridgeTx(
        request
      );
      ...
      expect(response.unsignedTx.value).toBe(
        ethers.utils.parseUnits("0.0000000001", 18).toString()
      );
    ...
    it("Native token with valid arguments is successful", async () => {
      ...
      expect(response.unsignedTx.value).toBe(
        amount.add(ethers.utils.parseUnits("0.0000000001", 18)).toString()
      );
      expect(response.unsignedTx.data).not.toBeNull();
    });
  ...
  describe("getFee", () => {
    ...
    const sourceChainGas: bigint = ethers.parseUnits("0.000000000015", 18);
    const approavalGas: bigint = ethers.parseUnits("0.0000000000055", 18);
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(voidRootProvider, "getFeeData")
        .mockImplementation(async () => ({
          lastBaseFeePerGas: BigNumber.from("50"),
          maxFeePerGas: BigNumber.from("100"),
          maxPriorityFeePerGas: BigNumber.from("50"),
      ...
      jest
        .spyOn(voidChildProvider, "getFeeData")
        .mockImplementation(async () => ({
          lastBaseFeePerGas: BigNumber.from("50"),
          maxFeePerGas: BigNumber.from("100"),
          maxPriorityFeePerGas: BigNumber.from("50"),
  ...
  describe("getTransactionStatus", () => {
    ...
    const DEPOSIT_SIG = ethers.utils.keccak256(
      ethers.utils.toUtf8Bytes("DEPOSIT")
    );
    const WITHDRAW_SIG = ethers.utils.keccak256(
    ...
    const mockDepositPayload = abiCoder.encode(
      ...
      [DEPOSIT_SIG, token, sender, recipient, amount]
    );
    const mockWithdrawPayload = abiCoder.encode(
      ["bytes32", "address", "address", "address", "uint256"],
      [WITHDRAW_SIG, token, sender, recipient, amount]
    );
    ...
    const mockERC20ContractFlowRate = {
      ...
      getPendingWithdrawalsLength: jest
        .fn()
        .mockImplementation(async () => BigInt(1)),
      withdrawalDelay: jest
        .fn()
        .mockImplementation(async () => BigInt(60 * 60 * 24)),
    };
    ...
    const mockERC20Contract = {
      ...
      getPendingWithdrawals: jest.fn().mockImplementation(async () => []),
      getPendingWithdrawalsLength: jest.fn().mockImplementation(async () => 0n),
    };
    ...
    it("returns the PENDING status for a deposit", async () => {
      ...
      const txHash =
        "0x5c192bf2b3be59de3a69877f6c71fd0affe6e1a1c05a75f51d4a60692001d8f3";
    ...
    it("returns the FLOW_RATE_CONTROLLED status for a withdrawal", async () => {
      ...
      jest
        ...
        .mockReturnValue(mockERC20ContractFlowRate as any);
      ...
      expect(result.transactions[0].status).toBe(
        StatusResponse.FLOW_RATE_CONTROLLED
      );
      expect(result.transactions[0].txHash).toBe(txHash);
      expect(result.transactions[0].token).toBe(token);
      expect(result.transactions[0].sender).toBe(sender);
    ...
    it("returns the FLOW_RATE_CONTROLLED status for multiple withdrawals", async () => {
      ...
      jest
        .spyOn(ethers, "Contract")
        .mockReturnValue(mockERC20ContractFlowRateMultiple as any);
      ...
      expect(result.transactions[0].recipient).toBe(recipient);
      expect(result.transactions[0].amount).toStrictEqual(amount);
      expect(result.transactions[1].status).toBe(
        StatusResponse.FLOW_RATE_CONTROLLED
      );
  ...
  describe("getFlowRateWithdrawTx", () => {
    ...
    it("returns the flowRate withdraw transaction when the index and recipient are valid", async () => {
      ...
      expect(result.unsignedTx?.to).toBe(
        bridgeConfig.bridgeContracts.rootERC20BridgeFlowRate
      );
      expect(result.unsignedTx?.value).toBe(0);
      ...
      expect(result.pendingWithdrawal.timeoutStart).toBe(
        defaultTimestamp.toNumber()
      );
      expect(result.pendingWithdrawal.timeoutEnd).toBe(
        defaultTimestamp.toNumber() + 60 * 60 * 24
      );
    });
    ...
    it("throws an error when the index is not valid", async () => {
      ...
      jest
        .spyOn(ethers, "Contract")
        .mockReturnValue(mockERC20ContractFlowRateNotFound as any);
    ...
    it("returns the flowRate info if the transcation is not ready to be withdrawn", async () => {
      ...
      jest
        .spyOn(ethers, "Contract")
        .mockReturnValue(mockERC20ContractFlowRateNotReady as any);
  ...
  describe("getPendingWithdrawals", () => {
    ...
    it("returns the flowRate pending withdrawals [1] when the recipient is valid", async () => {
      ...
      expect(result.pending[0].timeoutEnd).toBe(
        defaultTimestamp.toNumber() + 60 * 60 * 24
      );
    });
    ...
    it("returns the flowRate pending withdrawals [3] when the recipient is valid", async () => {
      ...
      const mockERC20ContractFlowRateThree = {
        ...
        getPendingWithdrawalsLength: jest
          .fn()
          .mockImplementation(async () => BigInt(3)),
        withdrawalDelay: jest
          .fn()
          .mockImplementation(async () => BigInt(60 * 60 * 24)),
      };
      ...
      jest
        .spyOn(ethers, "Contract")
        .mockReturnValue(mockERC20ContractFlowRateThree as any);
      ...
      expect(result.pending[0].timeoutStart).toBe(
        Number(defaultTimestamp * 1n)
      );
      expect(result.pending[0].timeoutEnd).toBe(
        Number(defaultTimestamp * 1n) + 60 * 60 * 24
      );
      ...
      expect(result.pending[1].timeoutEnd).toBe(
        Number(futureTimestamp) + 60 * 60 * 24
      );
      ...
      expect(result.pending[2].timeoutStart).toBe(
        Number(defaultTimestamp * 3n)
      );
      expect(result.pending[2].timeoutEnd).toBe(
        Number(defaultTimestamp * 3n) + 60 * 60 * 24
      );
    });
    ...
    it("returns empty pending array when no flowRated transactions found", async () => {
      ...
      jest
        .spyOn(ethers, "Contract")
        .mockReturnValue(mockERC20ContractFlowRateNone as any);
  ...
  describe("getFlowRateInfo", () => {
    ...
    const mockERC20ContractFlowRate = {
      ...
      withdrawalDelay: jest
        .fn()
        .mockImplementation(async () => withdrawalDelay),
      ...
      largeTransferThresholds: jest
        .fn()
        .mockImplementation(async () => largeTxThreshold),
    };
    ...
    it("returns the flow rate info for the multiple tokens", async () => {
      ...
      jest
        .spyOn(ethers, "Contract")
        .mockReturnValue(mockERC20ContractFlowRateMultiToken as any);
  ...
  describe("getTokenMapping", () => {
    ...
    const mockRootBridgeIMX = {
      rootTokenToChildToken: jest
        .fn()
        .mockImplementation(
          async () => bridgeConfig.bridgeContracts.rootChainIMX
        ),
    };
    ...
    const mockRootBridgeUSDC = {
      rootTokenToChildToken: jest
        .fn()
        .mockImplementation(async () => childUSDCToken),
    };
