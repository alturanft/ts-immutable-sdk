...
async function deposit() {
  ...
  if (
    ...
    if (childBridgeChildAddress === ethers.constants.AddressZero) {
      throw new Error(
        "token mapping incomplete, please wait for token to map to childBridge before depositing"
      );
    }
  ...
  if (approvalRes!.unsignedTx) {
    ...
    const approvalTxSig = await params.rootWallet.signTransaction(
      approvalRes!.unsignedTx
    );
    console.log("approvalTxSig", approvalTxSig);

    const sendApprovalRes = await params.rootWallet.provider.sendTransaction(
      approvalTxSig
    );
  ...
  depositRes!.unsignedTx.value = BigInt(depositRes!.unsignedTx.value);

  console.log("depositRes.unsignedTx");
  console.log(depositRes!.unsignedTx);

  console.log("signing deposit");
  const depositTxSig = await params.rootWallet.signTransaction(
    depositRes!.unsignedTx
  );
  console.log("depositTxSig", depositTxSig);

  const sendDepositRes = await params.rootWallet.provider.sendTransaction(
    depositTxSig
  );
