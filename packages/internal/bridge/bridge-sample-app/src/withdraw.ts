...
export async function withdraw() {
  ...
  const rootBridge: ethers.Contract = getContract(
    "RootERC20BridgeFlowRate",
    params.rootBridgeAddress,
    params.rootProvider
  );
  const childBridge: ethers.Contract = getContract(
    "ChildERC20Bridge",
    params.childBridgeAddress,
    params.childProvider
  );

  if (
    params.childToken.toUpperCase() !== "NATIVE" &&
    params.rootToken.toUpperCase() !== "NATIVE"
  ) {
    let rootBridgeChildAddress = await rootBridge.rootTokenToChildToken(
      params.rootToken
    );
    let childBridgeChildAddress = await childBridge.rootTokenToChildToken(
      params.rootToken
    );
    ...
    if (childBridgeChildAddress === ethers.constants.AddressZero) {
      throw new Error(
        "token mapping incomplete, please wait for token to map to childBridge before withdrawing"
      );
    }

    if (rootBridgeChildAddress !== childBridgeChildAddress) {
      throw new Error(
        `token mappings mismatch on rootBridge (${rootBridgeChildAddress}) & childBridge (${childBridgeChildAddress}).`
      );
    }
  }
  ...
  if (approvalRes!.unsignedTx) {
    ...
    const approvalTxSig = await params.childWallet.signTransaction(
      approvalRes!.unsignedTx
    );
    console.log("approvalTxSig", approvalTxSig);

    const sendApprovalRes = await params.childWallet.provider.sendTransaction(
      approvalTxSig
    );
  ...
  withdrawRes!.unsignedTx.value = BigInt(withdrawRes!.unsignedTx.value);
  ...
  const withdrawTxSig = await params.childWallet.signTransaction(
    withdrawRes!.unsignedTx
  );
  console.log("withdrawTxSig", withdrawTxSig);

  const sendWithdrawtRes = await params.childWallet.provider.sendTransaction(
    withdrawTxSig
  );
  ...
  while (!complete) {
    attempts++;
    const txStatusRes: TxStatusResponse =
      await tokenBridge.getTransactionStatus(txStatusReq);
    console.log(`TxStatusResponse attempt: ${attempts}`);
    console.log(
      util.inspect(txStatusRes, {
        showHidden: false,
        depth: null,
        colors: true,
      })
    ...
    if (
      txStatusRes.transactions[0].status === StatusResponse.COMPLETE ||
      txStatusRes.transactions[0].status ===
