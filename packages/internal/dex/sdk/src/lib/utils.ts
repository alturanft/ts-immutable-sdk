...
export const decimalsFunctionSig = ethers.utils
  .id("decimals()")
  .substring(0, 10);
...
export const toBigNumber = (
  amount: Uniswap.CurrencyAmount<Uniswap.Token>
): bigint => BigInt(amount.multiply(amount.decimalScale).toExact());
...
export const isERC20Amount = (
  amount: CoinAmount<Coin>
): amount is CoinAmount<ERC20> => amount.token.type === "erc20";

export const isNativeAmount = (
  amount: CoinAmount<Coin>
): amount is CoinAmount<Native> => amount.token.type === "native";

export const isNative = (token: Coin): token is Native =>
  token.type === "native";
...
export const addAmount = <T extends Coin>(
  a: CoinAmount<T>,
  b: CoinAmount<T>
) => {
...
export const subtractERC20Amount = (
  a: CoinAmount<ERC20>,
  b: CoinAmount<ERC20>
) => {
  // Make sure the ERC20s have the same address
...
export const subtractAmount = <T extends Coin>(
  a: CoinAmount<T>,
  b: CoinAmount<T>
) => {
  if (isERC20Amount(a) && isERC20Amount(b)) {
...
export const getDefaultDeadlineSeconds = (): number =>
